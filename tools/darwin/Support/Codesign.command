...
if [ "${PLATFORM_NAME}" == "iphoneos" ]; then
  ...
  ${GEN_ENTITLEMENTS} "org.vidonme.xbmc-ios" "${BUILT_PRODUCTS_DIR}/${WRAPPER_NAME}/${PROJECT_NAME}.xcent";
  ...
  if ! [ -z "${CODE_SIGN_IDENTITY}" ] && [ "${CODE_SIGN_IDENTITY}" != "iPhone Developer" ] && [ "${CODE_SIGN_IDENTITY}" != "Don't Code Sign"  ]; then
    ...
    do
      codesign -fvvv -s "${CODE_SIGN_IDENTITY}" -i org.vidonme.xbmc-ios `find ${CODESIGNING_FOLDER_PATH} -name "*.$binext" -type f` ${CODESIGNING_FOLDER_PATH}
    done
