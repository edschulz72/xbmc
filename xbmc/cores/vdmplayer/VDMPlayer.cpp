...
bool CVDMPlayer::QueueNextFile(const CFileItem& file)
{
	if (m_pPlayer)
	{
		...
		item.pszFile = file.GetPath().c_str();
		item.pszMimeType = file.GetMimeType().c_str();
...
void CVDMPlayer::GetGeneralInfo(std::string& strVideoInfo)
{
	CSingleLock lock(s_renderlock);
	if (!m_pPlayer)
		return;
...
static ExternalDirectoryHandle* Exfsys_GetDirectory(const char* path)
{
	...
	handle->pDirectory = CDirectoryFactory::Create(pathToUrl);
	...
	if (!handle->pDirectory->GetDirectory(pathToUrl, handle->itemList))
	{
...
static bool Exfsys_CreateDirectory(const char* path)
{
	...
	std::auto_ptr<IDirectory> pDirectory(CDirectoryFactory::Create(pathToUrl));

	if (pDirectory.get())
		return pDirectory->Create(pathToUrl);
...
static bool Exfsys_ExistsDirectory(const char* path)
{
	...
	if (pDirectory.get())
		return pDirectory->Exists(pathToUrl);
...
static bool Exfsys_RemoveDirectory(const char* path)
{
	...
	if (pDirectory.get())
		return pDirectory->Remove(pathToUrl);
...
const std::map<CStdString, CStdString>& CVDMPlayer::GetPlayerCoreVersion2()
{
	...
	std::vector<std::string> lines = StringUtils::Split(szPlayerCoreVersion, "\n");
	for (std::vector<std::string>::iterator line = lines.begin()
		; line != lines.end(); ++line)
	{
		std::vector<std::string> keyVal = StringUtils::Split(*line, ":");
		if (keyVal.size() == 2)
		{
...
void CVDMPlayer::UpdateViewMode(void)
{
	//s_dllPlayercore.Libplayercore_SetSettingInt(CS_VIDEO_VIEW_MODE, CMediaSettings::Get().GetCurrentVideoSettings().m_ViewMode);
}

void CVDMPlayer::UpdateZoomAmount(void)
{
	//s_dllPlayercore.Libplayercore_SetSettingFloat(CS_CUSTOM_ZOOM_AMOUNT, CMediaSettings::Get().GetCurrentVideoSettings().m_CustomZoomAmount);
}

void CVDMPlayer::UpdateVerticalShift(void)
{
	//s_dllPlayercore.Libplayercore_SetSettingFloat(CS_CUSTOM_VERTICAL_SHIFT, CMediaSettings::Get().GetCurrentVideoSettings().m_CustomVerticalShift);
}

void CVDMPlayer::UpdatePixelRatio(void)
{
	//s_dllPlayercore.Libplayercore_SetSettingFloat(CS_CUSTOM_PIXEL_RATIO, CMediaSettings::Get().GetCurrentVideoSettings().m_CustomPixelRatio);
}
