...
public:
  ...
  virtual void UpdateSubtitleSetting(int size, int style, int color, int align);
...
protected:
  ...
  bool IsVirtualFile( CStdString& strPath );
  CStdString DecodePathFromVirtualToReal( CStdString& strPath );

  void GetSMBUsernamePassword(CStdString &strFilePath);
...
private:
  ...
  ICorePlayerAudioCallback m_ICorePlayerAudioCallback; 
public:
  ...
  static const char* GetAppPackageName();
  static const char* GetPlayerCoreVersion();
  static const std::map<CStdString, CStdString>& GetPlayerCoreVersion2();
