...
{
	KODI::MESSAGING::ThreadMessageCallback callbackData;
	callbackData.callback = cb.callback;
	callbackData.userptr = cb.userptr;

	KODI::MESSAGING::CApplicationMessenger::GetInstance().PostMsg(TMSG_CALLBACK, 0, 0, static_cast<void*>(&callbackData));
}

void CVDMPlaytoolCallback::OnRenderFlush()
{
	KODI::MESSAGING::CApplicationMessenger::GetInstance().PostMsg(TMSG_RENDERER_FLUSH);
}
...
{
	KODI::MESSAGING::CApplicationMessenger::GetInstance().PostMsg(TMSG_GUI_ACTION, WINDOW_INVALID, -1, static_cast<void*>(new CAction(ACTION_TRIGGER_OSD)));
}
...
{
	KODI::MESSAGING::CApplicationMessenger::GetInstance().PostMsg(TMSG_SETVIDEORESOLUTION, RESOLUTION(res), forceUpdate ? 1 : 0, 0);
}

void CVDMPlaytoolCallback::OnSetFullScreenVideo(bool bFull)
{
	KODI::MESSAGING::CApplicationMessenger::GetInstance().PostMsg(TMSG_SETFULLSCREEN, bFull ? 1 : 0, 0, 0);
}
