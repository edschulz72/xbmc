...
private:
    ...
    {
        MethodJob(const std::shared_ptr<MethodPtr>& method
            , const std::shared_ptr<CResult>& result)
            : m_method(method)
            , m_result(result)
        {
          // NULL
        ...
        std::shared_ptr<MethodPtr>   m_method;
        std::shared_ptr<CResult>  m_result;
    };

public:
    MethodHolder(const std::shared_ptr<MethodPtr>& method)
...
bool SynchroMethodCall( const std::shared_ptr<MethodPtr> method);
