...
bool CVDMDialogLogin::ShowLoginTip()
{
	...
	if (!CVDMUserInfo::Instance().IsLogin() && 
		CSettings::GetInstance().GetString("usercenter.licenseid") != "Unlicensed" && 
		CSettings::GetInstance().GetString("usercenter.licenseid") != g_localizeStrings.Get(70092) &&
	...
	if (!CVDMUserInfo::Instance().IsLogin())
	{
		...
		if (CGUIDialogYesNo::ShowAndGetInput(70007, 70100, 70101, 0, bCanceled, 70020, 70014, 10000) && !bCanceled)
		{
	...
	return CVDMUserInfo::Instance().IsCurrentLicenseAvailable();
}
...
{
public:
	Method_Login(const std::string& strUserName, const std::string& strPassword)
	{
...
void CVDMDialogLogin::OnLoginClicked()
{
	...
	std::string strUserName = pUserNameControl->GetLabel2();
	std::string strPassword = pPasswordControl->GetLabel2();
	...
	if (!strUserName.empty() && !strPassword.empty())
	{
		...
		SynchroMethodCall(std::shared_ptr<MethodPtr>(new MethodPtr(pLogin)));
		if (CVDMUserInfo::Instance().IsLogin() && CVDMUserInfo::Instance().GetLoginErrorDescription().empty()/*may be switch user*/)
		{
			...
			if (CVDMUserInfo::Instance().IsCurrentLicenseAvailable())
			{
				if (CVDMUserInfo::Instance().IsCurrentLicenseTrial() && m_item->HasProperty("FirstLogin"))
				{
					...
					std::string strEndTime = StringUtils::Format("[COLOR=red]%s[/COLOR]", CVDMUserInfo::Instance().GetCurrentLicenseEndTime().c_str());
...
{
public:
	Method_Bind(const std::string& strKey, const std::string& strCodeID)
	{
		m_strKey = strKey;
...
bool Method_Bind::MethodCall()
{
	...
	CVDMUserInfo::Instance().GetUsernameAndPassword(strUserName, strPassword);
	std::string strLicenseID = CVDMUserInfo::Instance().IsCurrentLicenseTrial() ? "" : CVDMUserInfo::Instance().GetCurrentLicenseID();
	if (!m_strKey.empty())
	{
...
void CVDMDialogLogin::OnBindClicked()
{
	...
	if (!strKey.empty() || !strCodeID.empty())
	{
		...
		SynchroMethodCall(std::shared_ptr<MethodPtr>(new MethodPtr(pBind)));
		if (CVDMUserInfo::Instance().IsCurrentLicenseAvailable())
		{
			CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Info, g_localizeStrings.Get(70014), g_localizeStrings.Get(70015));
			Close();
