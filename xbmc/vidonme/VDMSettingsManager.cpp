...
void CVDMSettingsManager::OnSettingAction(const CSetting *setting)
{
	...
	if (settingId == "debugging.viewlog")
	{
		...
		CBuiltins::GetInstance().Execute(strExecute);
	}
	...
	{
		CBuiltins::GetInstance().Execute(StringUtils::Format("VDMOpenURL(%s)", CVDMRegionFeature::Get().GetWebSite().c_str()));
	}

	if (settingId == "upgrade.forum")
	{
		CBuiltins::GetInstance().Execute(StringUtils::Format("VDMOpenURL(%s)", CVDMRegionFeature::Get().GetForum().c_str()));
	}
