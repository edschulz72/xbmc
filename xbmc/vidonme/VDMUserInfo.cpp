...
{
public:
  CLoginJob( const std::string& strUserName, const std::string& strPassword )
  {
  ...
  std::string GetErrorDescription() { return m_strErrorDesc; } ;
...
static bool LoadVMCCountryCodeSetting( const std::string& filepath, int& nCode )
{
  CXBMCTinyXML xmlDoc;
...
bool CVDMUserInfo::GetUsernameAndPassword( std::string& strUserName, std::string& strPassword )
{
  if( m_strUserName.empty() || m_strPassword.empty() )
  {
    std::string strUserDataFile = CSpecialProtocol::TranslatePath(URIUtils::AddFileToFolder(CProfilesManager::GetInstance().GetUserDataFolder(), "userdata.xml"));
    ...
    if( !LoadUserInfoFromFile( strUserDataFile ) )
    {
	  ...
	  std::string strUserDataFile1 = "/sdcard/Android/data/org.vidonme.cloud.tv/userinfo/userinfo.xml";
...
bool CVDMUserInfo::LoadUserInfoFromFile( const std::string& filepath )
{
  CXBMCTinyXML xmlDoc;
  if (!xmlDoc.LoadFile(filepath))
  {
...
bool CVDMUserInfo::SaveUsernameAndPassword( const std::string& strUserName, const std::string& strPassword )
{
...
bool CVDMUserInfo::DoLogin( std::string strUserName, std::string strPassword, bool bWait /*true*/ )
{
  {
    CSingleLock lock(m_secUserInfo);
...
void CVDMUserInfo::WaitLoginInBackground( bool bShowUI )
{
  ...
  if( bShowUI )
  {
    ...
    SynchroMethodCall(std::shared_ptr<MethodPtr>(new MethodPtr(pWaitLogin)));
  }
  else
  {
    m_event.WaitMSec( 30000 );
...
std::string CVDMUserInfo::GetLoginErrorDescription()
{
  CSingleLock lock(m_secUserInfo);
...
std::string CVDMUserInfo::GetCurrentLicenseID()
{
  std::string strLicenseID;
  CSingleLock lock(m_secUserInfo);
...
bool CVDMUserInfo::IsFunctionAvailable( const std::string& strFuncID )
{
  CSingleLock lock(m_secUserInfo);
...
void CVDMUserInfo::SaveSettingsUserInfo()
{
  if( this->IsLogin() )
  {
    CSettings::GetInstance().SetString( "usercenter.name", m_strUserName );
    CSettings::GetInstance().SetString( "usercenter.email", m_strEmail );
    ...
    {
      CSettings::GetInstance().SetString( "usercenter.licenseid", g_localizeStrings.Get(70092) );
      CSettings::GetInstance().SetString( "usercenter.licensetime", g_localizeStrings.Get(70092) );
    }
...
std::string CVDMUserInfo::GetCurrentLicenseEndTime()
{
  CSingleLock lock(m_secUserInfo);
...
void CVDMUserInfo::NotifyUser()
{
  if( this->IsLogin() && !this->IsCurrentLicenseEmpty() )
  {
    ...
    else if( this->IsCurrentLicenseAvailable() )
    {
      ...
      {
        std::string strTip = StringUtils::Format(g_localizeStrings.Get(70017).c_str(), endtime.GetAsLocalizedDate().c_str());
